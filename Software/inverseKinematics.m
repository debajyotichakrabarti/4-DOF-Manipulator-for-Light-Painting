function [angles] = inverseKinematics(posMat, link_params, current_config)
    % Inputs:
    ...
    try link_params == -1;
        link_params = [15 15 15 4]; % [a1 a2 a3 a4]
    end
    ...
    if(store1 ~= 0)
        ...
        for i = 1:2  % i corresponds to each theta3
            ...
            t2 = [t2; t2_out]; % t2 is a vector, each row corr. particular theta3
        end
    else
        t2 = atan2(beta,alpha); % t2 is a scalar
    end
